...
Q3D.Config = {
	...
	bgColor: "#dfe0e2",              // null is sky, ffffff is white, 000000 is black
	...
	(function () {
		...
		app.measure = {
			...
			showResult: function () {
				...
				// Per-point profile data (distance & elevation)
				var profile = [];
				...
				if (path.length > 0) {
					...
					for (var i = 0; i < path.length; i++) {
						...
						var elev = p.z / zScale;
						var mapPt = app.scene.toMapCoordinates(p);
				...
				// Store raw data for programmatic access (e.g. window.app.measure.profile)
				this.profile = profile;

				// Aggregate totals for the summary table (same as before)
				if (path.length > 1) {
					var dxy;
					for (var j = path.length - 1; j > 0; j--) {
						dxy = vec2.copy(path[j]).distanceTo(path[j - 1]);
						dz = (path[j].z - path[j - 1].z) / zScale;
				...
				// Add per-point data table, chart, and export buttons if there are points
				if (profile.length > 0) {
					// Left-bottom: scrollable data table
					html += '<div class="measure-container"><table class="measure">';
					html += "<tr><td>#</td><td>Distance (m)</td><td>Elevation (m)</td><td>X</td><td>Y</td></tr>";
					for (var k = 0; k < profile.length; k++) {
						var row = profile[k];
						html += "<tr><td>" + row.index + "</td><td>" +
							this.formatLength(row.distance) + "</td><td>" +
							this.formatLength(row.elevation) + "</td><td>" +
							row.x.toFixed(3) + "</td><td>" +
							row.y.toFixed(3) + "</td></tr>";
					}
					html += "</table></div>";
				...
				if (exportBtn) {
					...
					exportBtn.addEventListener("click", function () {
						if (!self.profile || !self.profile.length) return;

						var csvRows = [];
						csvRows.push("index,distance_m,elevation_m,x,y,z");
						self.profile.forEach(function (p) {
							csvRows.push([
								p.index,
								self.formatLength(p.distance),
								self.formatLength(p.elevation),
								p.x.toFixed(5),
								p.y.toFixed(5),
								p.z.toFixed(3)
							].join(","));
				...
				if (canvas && this.profile && this.profile.length > 1) {
					...
					// Compute ranges
					var minX = this.profile[0].distance;
					var maxX = this.profile[this.profile.length - 1].distance;
					var minY = Infinity;
					var maxY = -Infinity;
					for (var ii = 0; ii < this.profile.length; ii++) {
						var e = this.profile[ii].elevation;
						if (e < minY) minY = e;
						if (e > maxY) maxY = e;
					}
					if (minY === maxY) {
					...
					var xScale = (w - 2 * padding) / (maxX - minX);
					var yScale = (h - 2 * padding) / (maxY - minY);
					...
					ctx.lineWidth = 2;
					ctx.beginPath();
					for (var jj = 0; jj < this.profile.length; jj++) {
						var pnt = this.profile[jj];
						var x = padding + (pnt.distance - minX) * xScale;
						var y = h - padding - (pnt.elevation - minY) * yScale;
						if (jj === 0) ctx.moveTo(x, y);
						else ctx.lineTo(x, y);
					}
					ctx.stroke();
